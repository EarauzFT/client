...
const Chat = () => {
	...
	const getChat = async (id) => {
		setChatMessages([]);
		const chatInfo = await (await fetch(`http://localhost:3000/chat/${id}`)).json();
		setChatInfo(chatInfo.body[0]);
		const chatMessages = await (await fetch(`http://localhost:3000/message?chat=${id}`)).json();
		setChatMessages(chatMessages.body);
	};
